...
*/
function appendText($textarea, insertedText, range) {
  // firefox has different text insertion method, so need to add conditional
  if (isFirefox()) {
    $textarea.value = $textarea.value.substring(0, range.start)
      + insertedText
      + $textarea.value.substring(range.end);
  } else {
 ...
 * Because some browsers treat new lines as two characters (\r\n) instead of one character (\n)
 */
...
function setCaretToPos($textarea, selectionStart, selectionEnd) {
  ...
  if ($textarea.setSelectionRange) {
    $textarea.setSelectionRange(
      adjustOffset($textarea, selectionStart),
      adjustOffset($textarea, selectionEnd),
    );
  // for IE
  ...
  const lineNumber = $textarea.value.substring(0, $textarea.selectionStart).split('\n').length;
...
/**
 * Transform a textarea into Markdown editor
 * @param Node $textarea
...
function hMarkdownEditor($textarea, args = {}) {
  ...
  [...$buttons].forEach(($button) => {
    $button.addEventListener('click', (e) => {
      e.preventDefault();
      onButtonClick(e.currentTarget, $textarea);
    });
  });
}
